...
// Hooks.on('pf1PreDisplayActionUse', async ([actionUse]) => {
//     const { action, actor, item, shared } = actionUse;

//     const { targets } = shared;
//     const updateMap = targets.reduce((acc, curr) => ({
//         ...acc,
//         [curr.actorData._id]: {
//             currentHp: target.actorData.data.attributes.hp.value,
//             maxHp: target.actorData.data.attributes.hp.max,
//             amounts: [],
//         },
//     }));

//     for (const attack of shared?.templateData?.attacks || []) {
//         if (!attack?.isHealing) {
//             continue;
//         }

//         const amount = attack.damage?.total;
//         if (!amount) {
//             continue;
//         }
//     }

//     const healed = shared.templateData
// })
